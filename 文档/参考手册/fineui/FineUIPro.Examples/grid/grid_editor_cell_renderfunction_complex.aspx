<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="grid_editor_cell_renderfunction_complex.aspx.cs"
    Inherits="FineUIPro.Examples.grid.grid_editor_cell_renderfunction_complex" %>
...
<body>
    <form id="form1" runat="server">
        ...
        <f:Grid ID="Grid1" ShowBorder="true" ShowHeader="true" Title="表格（单击编辑）" EnableCollapse="true" Width="850px"
            ...
            AllowCellEditing="true" ShowSelectedCell="true" ClicksToEdit="1">
            <Columns>
    ...
    <script>
        ...
        function renderName(value, params) {
            if (params.rowValue["EntranceYear"] >= 2003) {
                params.td.addClass('highlight');
            }
            return value;
        }

        function onGridAfterEdit(event, value, params) {
            if (params.column.id === 'EntranceYear') {
                var nameCell = params.tr.find('.f-grid-cell-Name');
                if (value >= 2003) {
                    nameCell.addClass('highlight');
                } else {
                    nameCell.removeClass('highlight');
                }
            }
        }
        ...
        //    F(gridClientID).on('afteredit', function (event, value, params) {
        //        if (params.column.id === 'EntranceYear') {
        //            var nameCell = params.tr.find('.f-grid-cell-Name');
        //            if (value >= 2003) {
        //                nameCell.addClass('highlight');

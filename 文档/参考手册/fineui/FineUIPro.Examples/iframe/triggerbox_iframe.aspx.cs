...
namespace FineUIPro.Examples.iframe
{
    public partial class triggerbox_iframe : PageBase
    {
        ...
        {
            Notify("触发了 Window1 的关闭事件！");
        }

        protected void Button1_Click(object sender, EventArgs e)
        {
            Notify(String.Format("第一个触发器输入框的值是：{0} <br/>第一个隐藏输入框的值是：{1}", TriggerBox1.Text, HiddenField1.Text));
        }


        protected void Window2_Close(object sender, WindowCloseEventArgs e)
        {
            Notify("触发了 Window2 的关闭事件！");
        }

        protected void Button2_Click(object sender, EventArgs e)
        {
            Notify(String.Format("第二个触发器输入框的值是：{0}<br/>第二个隐藏输入框的值是：{1}", TriggerBox2.Text, HiddenField2.Text));
        }

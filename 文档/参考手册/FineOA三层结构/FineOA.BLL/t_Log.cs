...
namespace FineOA.BLL
{
	...
	{
		private readonly It_Log dal=DataAccess.Createt_Log();
		public t_Log()
		{}
		#region  BasicMethod
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public FineOA.Model.t_Log GetModelByCache(int FItemId)
		{
			
			string CacheKey = "t_LogModel-" + FItemId;
			...
			return (FineOA.Model.t_Log)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<FineOA.Model.t_Log> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<FineOA.Model.t_Log> DataTableToList(DataTable dt)
		{
			List<FineOA.Model.t_Log> modelList = new List<FineOA.Model.t_Log>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				FineOA.Model.t_Log model;

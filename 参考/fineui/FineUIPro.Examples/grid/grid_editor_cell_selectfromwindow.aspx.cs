...
{
    public partial class grid_editor_cell_selectfromwindow : PageBase
    {
        ...
        protected void Button2_Click(object sender, EventArgs e)
        {
            ...
            labResult.Text = "用户修改的数据：" + Grid1.GetModifiedData().ToString(Newtonsoft.Json.Formatting.None)
                + "<br/>" + "用户删除的数据：" + new JArray(Grid1.GetDeletedList()).ToString(Newtonsoft.Json.Formatting.None);
        ...
        {
            int[] selectedCell = Grid1.SelectedCell;
            if (selectedCell != null)
            {
                PageContext.RegisterStartupScript(Window1.GetShowReference("grid_editor_cell_selectfromwindow_iframe.aspx?EditingRowIndex=" + selectedCell[0]));
            }
        }
        ...
        protected void Window1_Close(object sender, WindowCloseEventArgs e)
        {
            if (!String.IsNullOrEmpty(e.CloseArgument))
            {
                ...
                if (dataObj != null)
                {
                    int editingRowIndex = dataObj.Value<int>("EditingRowIndex");
                    ...
                    sb.Append(Grid1.GetUpdateCellValueReference(editingRowIndex, "Name", name));
                    sb.Append(Grid1.GetUpdateCellValueReference(editingRowIndex, "Gender", gender));
                    sb.Append(Grid1.GetUpdateCellValueReference(editingRowIndex, "EntranceYear", entranceYear));
                    sb.Append(Grid1.GetUpdateCellValueReference(editingRowIndex, "EntranceDate", entranceDate));
                    sb.Append(Grid1.GetUpdateCellValueReference(editingRowIndex, "AtSchool", atSchool.ToString().ToLower()));
                    sb.Append(Grid1.GetUpdateCellValueReference(editingRowIndex, "Major", major));

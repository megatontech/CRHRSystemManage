<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="grid_editor_cell_datachange.aspx.cs"
    Inherits="FineUIPro.Examples.grid.grid_editor_cell_datachange" %>
...
<body>
    <form id="form1" runat="server">
        ...
        <f:Grid ID="Grid1" ShowBorder="true" ShowHeader="true" Title="表格（双击编辑）" EnableCollapse="true" Width="850px"
            ...
            <Listeners>
                <f:Listener Event="datachange" Handler="onGridDataChange" />
            </Listeners>
        </f:Grid>
        <br />
        ...
        <br />
        注：“保存数据”按钮默认禁用，当有数据变化时（删除行，新增行，修改单元格）启用。
    </form>
    <script>
        ...
        function onGridDataChange() {
            ...
            // 如果没有删除行，没有修改数据（新增行，修改现有数据）
            if (!grid.getDeletedRows().length && !grid.getModifiedData().length) {
                submitBtn.disable();
            } else {
                submitBtn.enable();
            }

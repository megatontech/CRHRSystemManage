...
namespace FineOA.BLL
{
	...
	{
		private readonly It_BillNo dal=DataAccess.Createt_BillNo();
		public t_BillNo()
		{}
		#region  BasicMethod
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public FineOA.Model.t_BillNo GetModelByCache(int FItemId)
		{
			
			string CacheKey = "t_BillNoModel-" + FItemId;
			...
			return (FineOA.Model.t_BillNo)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<FineOA.Model.t_BillNo> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<FineOA.Model.t_BillNo> DataTableToList(DataTable dt)
		{
			List<FineOA.Model.t_BillNo> modelList = new List<FineOA.Model.t_BillNo>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				FineOA.Model.t_BillNo model;
